...
print_arp_packet(struct rte_arp_hdr *arp_h)
{
	printf("  ARP:  hrd=%d proto=0x%04x hln=%d "
			"pln=%d op=%u (%s)\n",
			CHECK_ENDIAN_16(arp_h->arp_hardware),
			CHECK_ENDIAN_16(arp_h->arp_protocol), arp_h->arp_hlen,
			arp_h->arp_plen, CHECK_ENDIAN_16(arp_h->arp_opcode),
			arp_op_name(arp_h->arp_opcode));

	if (CHECK_ENDIAN_16(arp_h->arp_hardware) != RTE_ARP_HRD_ETHER) {
		printf("incorrect arp_hrd format for IPv4 ARP (%d)\n",
				(arp_h->arp_hardware));
	} else if (CHECK_ENDIAN_16(arp_h->arp_protocol) != RTE_ETHER_TYPE_IPV4) {
		printf("incorrect arp_pro format for IPv4 ARP (%d)\n",
				(arp_h->arp_protocol));
	} else if (arp_h->arp_hlen != 6) {
		printf("incorrect arp_hln format for IPv4 ARP (%d)\n",
				arp_h->arp_hlen);
	} else if (arp_h->arp_plen != 4) {
		...
		void *arg)
{
	...
	if (eth_h->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP)) {
		struct rte_arp_hdr *arp_h = (struct rte_arp_hdr *)((char *)eth_h +
								sizeof(struct rte_ether_hdr));
		if (ARPICMP_DEBUG)
			print_arp_packet(arp_h);
		...
		if (CHECK_ENDIAN_16(arp_h->arp_hardware) != RTE_ARP_HRD_ETHER) {
			...
			} else if (arp_h->arp_opcode == rte_cpu_to_be_16(RTE_ARP_OP_REQUEST)) {
			...
			} else if (arp_h->arp_opcode == rte_cpu_to_be_16(RTE_ARP_OP_REPLY)) {
				/* Process ARP_RSP */
				if (ARPICMP_DEBUG) {
					printf(
